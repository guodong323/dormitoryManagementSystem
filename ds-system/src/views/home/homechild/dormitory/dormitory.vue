...
export default {
  ...
  data() {
    return {
      ...
      addFormRules: {
        housename: [{ required: true, message: "请输入楼栋", trigger: "blur" }],
      },
      deleteFormRules: {
        housename: [{ required: true, message: "请输入楼栋", trigger: "blur" }],
      },
      addmsgFormRules: {
        ...
        floor_id: [{ required: true, message: "请输入楼栋", trigger: "blur" }],
      },
      isshow: true,
      btn_index: "",
      isshow_who: true,
  ...
  methods: {
    ...
    bakcToAll() {
      // this.$notify.success("信息请求成功");
      this.$http
        .post("message/getallmessage").then((res) => {
