<template>
  <div>
    ...
    >
      ...
      <!-- 确认密码 -->
      <el-form-item prop="againpassward">
        <el-input
          placeholder="确认密码"
...
export default {
  ...
  data() {
    return {
      loginform: { username: "", password: "", againpassword: "", radio: "", },
      //校验规则
      radio: "1",
      loginrules: {
        username: [
  ...
  methods: {
    ...
    registerUser() {
        ...
        this.$http
        .post("user/register", this.$qs.stringify(this.loginform))
        .then((res) => {
        if (res == "registerNO")
